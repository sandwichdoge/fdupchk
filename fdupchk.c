...
typedef struct {
	char fpath[1024];
	char fname[128];
	unsigned int fsize;  //4gb max file

} file_t;


//store info of a file [NAME+SIZE] to buffer struct.
file_t *file_get_info(char *filepath, int mode)
{
	...
	//TODO: add HASH method here to compare file content since fsize caps at 4gb?
	return pfile_struct_buf;
}

int fcmp(file_t *pfile1, file_t *pfile2, short mode)
{
	byte ret = 0;
	char buf1[1024];
	char buf2[1024];
...
unsigned int count_items(char *dir)
{
	...
	while (entry = readdir(dir_stream)) {
		...
		if (entry->d_type == DT_DIR) {
			if (strcmp(entry->d_name, "..") == 0 || strcmp(entry->d_name, ".") == 0) continue;
				...
				count += count_items(full_entry_path);
			}
		}
...
//traverse folder and list all files in an array of pointers to structs.
int traverse_folder(file_t **file_table, char *dir, int mode)
{
	char full_entry_path[1024];
	DIR *dir_stream = opendir(dir);
	...
	while (entry = readdir(dir_stream)) {
		...
		if (entry->d_type == DT_DIR) {
			if (strcmp(entry->d_name, "..") == 0 || strcmp(entry->d_name, ".") == 0) continue;
				...
				traverse_folder(file_table, full_entry_path, mode);
			}
		}
		else if (entry->d_type == DT_REG) {  //if entry is a regular file -> index it to file table.
...
int main(int argc, char **argv)
{
	...
	file_t **file_table;
	printf("search dir: %s\n", search_dir);
	unsigned int items = count_items(search_dir);
	
	printf("counted %d files in directory %s.\n", items, search_dir);
	file_table = malloc(items * sizeof(file_t*));
	traverse_folder(file_table, search_dir, mode);
